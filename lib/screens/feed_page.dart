...
import 'package:instagram/utils/profile_img_path.dart';
...
*/
  Padding _postCaption(BuildContext context, int index) {
    return Padding(
      padding: const EdgeInsets.symmetric(
          horizontal: common_gap, vertical: common_xs_gap), // horizontal: 양옆, vertical: 위아래
      child: Comment(
  ...
  Padding _postLikes() {
    return Padding(
      padding: const EdgeInsets.only(left: common_gap), // EdgeInsets.all(14) => 전체로 패딩 줄 수 있음
      child: Text(
        '80 likes',
  ...
  CachedNetworkImage _postImage(int index) {
    return CachedNetworkImage(
      ...
      imageBuilder: (BuildContext context, ImageProvider imageProvider) =>
          AspectRatio(
        aspectRatio: 1,   //  1(가로)/1(세로) 한 값 1 이 들어가 있는 것이다. 2 는 2(가로)/1(세로) 비율이다.
        child: Container(
